...
<details open>
<summary><b>Q5. $O(1)$ 상수 공간 캐시(`history_vessel`) 구조에서 문맥(Context)이 무한히 길어질 때 '기존 지식 유실이나 정보 압착 붕괴' 현상은 어떻게 제어하나요?</b></summary>
  - **A:** 표준 트랜스포머의 KV 캐시는 시퀀스 축($N$)을 따라 데이터를 물리적으로 계속 이어 붙이기 때문에 정보가 유실되지 않는 대신 VRAM이 폭발합니다. 반면, 본 엔진은 새로운 단일 토큰이 유입될 때마다 그 파동 전하량을 외적(Matrix Outer Product) 연산하여 고정 차원의 격자 용기인 `history_vessel`에 인플레이스 가산(`vessel + delta_vessel`) 수착시킵니다. 
  - 이 과정은 수리물리학적으로 **'연속체 매니폴드 상에서의 가상 모멘트 적분 적산식'**을 모사합니다. 즉, 과거의 정보가 선형적으로 유실되는 것이 아니라, 푸리에 직교 기저 평면 내부의 기하학적 파동 간섭 전하량으로 상시 중첩·축적되는 형태를 취합니다. 이로 인해 문맥이 확장되어도 물리적인 메모리 점유율을 변화시키지 않으면서, 역전파 자동 미분 시 도함수 전하량이 끊기지 않고 과거 전체 맥락의 지능을 수호해 낼 수 있는 물리적 복원 토폴로지가 구성 되는것이 저의 이론입니다.
