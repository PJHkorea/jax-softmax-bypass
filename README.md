...
<details open>
<summary><b>Q5. O(1) 상수 공간 캐시(<code>history_vessel</code>) 구조에서 문맥(Context)이 무한히 길어질 때 '기존 지식 유실이나 정보 압착 붕괴' 현상은 어떻게 제어하나요?</b></summary>

* **What was wrong :** 표준 트랜스포머의 KV 캐시는 시퀀스 축($N$)을 따라 토큰 시퀀스를 물리적으로 계속 이어 붙이기 때문에 정보 유실이 없는 대신 VRAM이 폭발하며 OOM을 일으킵니다. 반면 일반적인 고정 차원 압착 구조는 윈도우 밖의 과거 정보가 선형적으로 유실되거나 가우시안 평탄화로 인해 랭크가 붕괴되는 치명적인 지능 손실을 겪었습니다.
* **How we fixed it :** 본 엔진은 새로운 단일 토큰이 유입될 때마다 그 파동 전하량을 외적(Matrix Outer Product) 연산하여 고정 차원의 격자 용기인 `history_vessel`에 인플레이스 가산(`vessel + delta_vessel`) 수착시키는 물리적 연속체 적분 기전을 구현했습니다. 과거의 정보가 멸실되는 것이 아니라 푸리에 직교 기저 평면 내부의 기하학적 파동 간섭 전하량으로 상시 중첩·축적되므로, 수만 토큰의 컨텍스트 확장 시에도 메모리 점유율을 $O(1)$로 동결한 채 도함수 전하량 누수 없이 전역 맥락의 지능을 복원및 보호한다는 것이 저의 이론 입니다.
</details>
